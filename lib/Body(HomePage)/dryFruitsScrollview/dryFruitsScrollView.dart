...
import 'package:final_local_shouts/ModelClass/dryFruitsModelClass.dart';
import 'package:flutter/material.dart';
import 'package:http/http.dart' as http;

class dryFruitsScrollView extends StatefulWidget {
  const dryFruitsScrollView({super.key});

  @override
  State<dryFruitsScrollView> createState() => _dryFruitsScrollViewState();
}

class _dryFruitsScrollViewState extends State<dryFruitsScrollView> {
  Future<ProductsModelDryFruits> getProductsApi() async {
    //create your own api
    final response = await http.get(Uri.parse(
        'https://localshouts.com/api/catProduct?category=dry-fruits'));
    var data = jsonDecode(response.body.toString());
    if (response.statusCode == 200) {
      return ProductsModelDryFruits.fromJson(data);
    } else {
      return ProductsModelDryFruits.fromJson(data);
    }
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      height: 180,
      child: FutureBuilder<ProductsModelDryFruits>(
