...
import 'package:final_local_shouts/ModelClass/FruitsModelClass.dart';
import 'package:flutter/material.dart';
import 'package:http/http.dart' as http;

class FruitsScrollView extends StatefulWidget {
  const FruitsScrollView({super.key});

  @override
  State<FruitsScrollView> createState() => _FruitsScrollViewState();
}

class _FruitsScrollViewState extends State<FruitsScrollView> {
  Future<ProductsModelFruits> getProductsApi() async {
    //create your own api
    final response = await http.get(Uri.parse(
        'https://localshouts.com/api/catProduct?category=fruits'));
    var data = jsonDecode(response.body.toString());
    if (response.statusCode == 200) {
      return ProductsModelFruits.fromJson(data);
    } else {
      return ProductsModelFruits.fromJson(data);
    }
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      height: 180,
      child: FutureBuilder<ProductsModelFruits>(
